...
global contract
one={
    'from':'0xb719ceFa2e7BDceBB208e07BB3225691EA8BA978'  # account address
}
...
def readDetails(contract_type):
    ...
    deployed_contract_address = '0xa92AfFCC366581091b40f114E4c699B4343Ff263' # contract address 
    ...
    if contract_type == 'schoolcompany':
        details = contract.functions.getUniversityCompany().call(one)
    if contract_type == 'enrollstudent':
        details = contract.functions.getStudent().call(one)
    if contract_type == 'credential':
        details = contract.functions.getCredential().call(one)
...
def saveDataBlockChain(currentData, contract_type):
    ...
    if contract_type == 'schoolcompany':
        details+=currentData
        msg = contract.functions.addUniversityCompany(details).transact(one)

    if contract_type == 'enrollstudent':
        details+=currentData
        msg = contract.functions.enrollStudent(details).transact(one)

    if contract_type == 'credential':
        details+=currentData
        msg = contract.functions.setCredentialData(details).transact(one)
    
    if contract_type == 'accessrequest':
        details+=currentData
        msg = contract.functions.setAccessRequest(details).transact(one)
...
def saveDataBlockChain1(currentData, contract_type):
    ...
    contract = web3.eth.contract(address=deployed_contract_address, abi=contract_abi)
    if contract_type == 'credential':
        msg = contract.functions.setCredentialData(currentData).transact(one)
...
def ViewStudents(request):
    if request.method == 'GET':
        ...
        output = '<table class="data-table" align="center" width="100%">'
        font = '<font size="" >'
...
def UpdateCertificateAction(request):
    if request.method == 'POST':
        ...
        with open('StudentCredentialApp/static/certificates/update'+cid+".png", "wb") as file:
            file.write(myfile)
        file.close()    
        '''
        myfile = pickle.dumps(myfile)
        ...
        if flag == True:
            ...
            data += school_name+"#"+"update"+cid+"#"+sid+"#"+certificate+"#"+issue_date+"#"+filename+"#"+hashcode+"\n"
...
def SendAccessRequest(request):
    if request.method == 'GET':
        ...
        for i in range(len(arr)-1):
            ...
            output += "<td>"+font+array[4]+"</td>"
            output += "<td>"+font+array[5]+"</td>"
            output += '<td style="background-color: #f8f9fa; text-align: center;"><a href="SendRequest?t1=' + array[1] + '" style="font-size: 1.2rem; color: white; background-color: #3445db; padding: 8px 12px; border-radius: 5px; text-decoration: none; transition: background-color 0.3s ease;">Click Here</a></td></tr>'        
            context= {'data':output}        
    ...
    if request.method == 'GET':
        ...
        for i in range(len(arr)-1):
            ...
            if array[0] == username:
                ...
                output += "<td>"+font+array[1]+"</td>"
                output += "<td>"+font+array[2]+"</td>"
                output += '<td><a href=\'GrantAccessAction?t1=' + array[0] + '&t2=' + array[1] + '\' class="grant-access-button"><font size=3 color="white">Click Here</font></a></td></tr>'
        context= {'data':output}        
